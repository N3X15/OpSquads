...
function ENT:SpawnFunction(ply, tr)
	...
	npc:Spawn()
	npc.OPSQUADWillDropAlphaGland=true
	npc:Activate()
	npc.HasAJackyAllegiance=true
	if(Friendly)then
		npc.JackyAllegiance="Human"
	else
		npc.JackyAllegiance="XenoCreature"
	end
	npc:SetKeyValue("SquadName",npc.JackyAllegiance)
//	npc:SetCollisionGroup(3)            --this collision thing isn't necessary anymore since they can't hurt eachother
	...
	npc.IsACavernGuard=true --this is so when the entity dies his ragdoll gets the cavernguard material
...
//		npc:SetCollisionGroup(3)
	...
	timer.Simple(0.15,function()   --DERP
		local npc=ents.Create("npc_antlion")
...
//		npc:SetCollisionGroup(3)
	...
	timer.Simple(0.35,function()  --DERP
		local npc=ents.Create("npc_antlion")
		npc:SetPos(selfpos+Vector(-75,0,0))
		npc:SetAngles(Angle(0,180,0))
		npc:SetKeyValue("startburrowed","1")
