	...
	if(IsValid(self.Owner))then
		...
		if not(IsValid(Enemy))then
			if(Act==ACT_IDLE)then
				if(self.RoundsInPulseMag<5)then
					self:Reload(true)
				end
			end
			for key,found in pairs(ents.FindInSphere(self.Owner:GetPos(),8000))do
...
function SWEP:NPCShoot_Primary(ShootPos,ShootDir)
	if(not(self.Reloading)and not(self.Aiming))then
		...
		if(IsValid(Enemy))then
			...
			//local Time=(self.Owner:GetPos()-Enemy:GetPos()):Length()/1500+Enemy:GetPhysicsObject():GetVelocity():Length()/250+1
			local Time=math.Rand(0.9,1.1)*(Distance^0.75/150)+Speed/300+1
			self.Aiming=true
...
function SWEP:Shoot(ShootPos,ShootDir)
	...
	if(self.RoundsInPulseMag>0)then
		...
		local EnemyPos=Enemy:LocalToWorld(Enemy:OBBCenter())+VectorRand()*math.Rand(0,Dist/250)+self:HeightCorrection()
		...
		if(EyeTrace.Hit)then
			WorldSound("lolsounds/shot_echo.wav",EyeTrace.HitPos,110,100)
		
			local Bewlat={}
			Bewlat.Num=1
			Bewlat.Damage=math.Rand(150,175)
			Bewlat.Force=4000
			Bewlat.Src=ShootPos
			Bewlat.Dir=Dir
