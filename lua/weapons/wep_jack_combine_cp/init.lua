...
function SWEP:ExtraThink()
	if(IsValid(self.Owner))then	
		...
		if not(self.Reloading)then
			...
			if not(IsValid(Enemy))then
				...
				if not(self.Reloading)then
					if(Act==ACT_IDLE)then
						if(self.RoundsInPulseMag<20)then
							self:Reload(true)
						end
					end
				end
			...
			self.Owner:SetPlaybackRate(1.3)
		end
	end
end
...
function SWEP:Shoot(ShootPos,ShootDir)
	...
	if(self.RoundsInPulseMag>0)then
		...
		WorldSound("NPC_FloorTurret.Shoot",ShootPos,100,100)
		WorldSound("lolsounds/combinebr_close.wav",ShootPos,60,130)
		...
		local EnemyPos=Enemy:LocalToWorld(Enemy:OBBCenter())+VectorRand()*math.Rand(0,Dist/12)+self:HeightCorrection()
