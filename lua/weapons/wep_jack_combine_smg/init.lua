...
function SWEP:ExtraThink()
	if(IsValid(self.Owner))then	
		if not(self.Reloading)then
			...
			if not(IsValid(Enemy))then
				...
				if not(self.Reloading)then
					if(Act==ACT_IDLE)then
						if(self.RoundsInPulseMag<75)then
							self:Reload(true)
						end
					end
				end
			...
			self.Owner:SetPlaybackRate(1.2)
		end
	end
end
...
function SWEP:Shoot(ShootPos,ShootDir)
	...
	if(self.RoundsInPulseMag>0)then
		...
		local EnemyPos=Enemy:LocalToWorld(Enemy:OBBCenter())+VectorRand()*math.Rand(0,Dist/7)+self:HeightCorrection()
