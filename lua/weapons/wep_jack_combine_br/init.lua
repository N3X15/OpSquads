		...
		if not(self.Reloading)then
			...
			if not(IsValid(Enemy))then
				if(Act==ACT_IDLE)then
					if(self.RoundsInPulseMag<15)then
						self:Reload()
					end
				end
			else
				...
				if not(Enemy==self.CurrentTarget)then --if we either changed target or acquired a new target
					self.NextShootTime=CurTime()+0.5
...
function SWEP:Shoot(ShootPos,ShootDir)
	...
	if(self.RoundsInPulseMag>0)then
		...
		local EnemyPos=Enemy:LocalToWorld(Enemy:OBBCenter())+VectorRand()*math.Rand(0,Dist/60)+self:HeightCorrection()
