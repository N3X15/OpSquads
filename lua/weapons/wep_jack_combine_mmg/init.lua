...
function SWEP:ExtraThink()
	if(IsValid(self.Owner))then
		if not(self.Reloading)then
			...
			self.Owner:SetPlaybackRate(0.25)
		end
	end
end
...
function SWEP:Shoot(ShootPos,ShootDir)
	...
	if(self.RoundsInPulseMag>0)then
		WorldSound("lolsounds/combinebr_close.wav",ShootPos,85,math.Rand(100,120))
		...
		local EnemyPos=Enemy:LocalToWorld(Enemy:OBBCenter())+VectorRand()*math.Rand(0,Dist/40)+self:HeightCorrection()
...
function SWEP:Reload()
	...
	timer.Simple(TimerTable[1]+AddTime,function()
		if(IsValid(self.Owner))then
			...
			self.Owner:EmitSound("lolsounds/combine_smg_ar_br_magremove.wav",100,80)
		end
	end)
	...
	timer.Simple(TimerTable[3]+AddTime,function()
		if(IsValid(self.Owner))then
			...
			self.Owner:EmitSound("lolsounds/combine_smg_ar_br_maginsert.wav",100,80)
		end
	end)
	timer.Simple(TimerTable[4]+AddTime,function()
		if(IsValid(self))then
			self.Owner:EmitSound("lolsounds/combine_smg_ar_br_cockback.wav",100,80)
		end
	end)
	timer.Simple(TimerTable[5]+AddTime,function()
		if(IsValid(self))then
			self.Reloading=false
			self.Owner:EmitSound("lolsounds/combine_smg_ar_br_cockforward.wav",100,80)
		end
	end)
