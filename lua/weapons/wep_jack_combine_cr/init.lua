		...
		if not(self.Reloading)then
			...
			if not(IsValid(Enemy))then
				if(Act==ACT_IDLE)then
					if(self.RoundsInPulseMag<22)then
						self:Reload()
					end
				end
			else
				...
				if not(Enemy==self.CurrentTarget)then --if we either changed target or acquired a new target
					self.NextShootTime=CurTime()+0.35
...
function SWEP:NPCShoot_Primary(ShootPos,ShootDir)
	if((self.NextShootTime<CurTime())and not(self.Reloading))then self:Shoot(ShootPos,ShootDir) end
end
...
function SWEP:Shoot(ShootPos,ShootDir)
	...
	if(self.RoundsInPulseMag>0)then
		...
		WorldSound("Weapon_AR2.Single",ShootPos,100,100)

		local ShootAng=ShootDir:Angle()
		local angpos=self:GetAttachment(self:LookupAttachment("muzzle"))
		local ShootOrigin=angpos.Pos+angpos.Ang:Forward()*22+angpos.Ang:Up()*5
		...
		local EnemyPos=Enemy:LocalToWorld(Enemy:OBBCenter())+VectorRand()*math.Rand(0,Dist/45)+self:HeightCorrection()
