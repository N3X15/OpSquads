			...
			if not(IsValid(Enemy))then
				...
				if not(self.Reloading)then
					if(Act==ACT_IDLE)then
						if(self.RoundsInPulseMag<40)then
							self:Reload(true)
						end
					end
				end
...
function SWEP:Shoot(ShootPos,ShootDir)
	...
	if(self.RoundsInPulseMag>0)then
		...
		local EnemyPos=Enemy:LocalToWorld(Enemy:OBBCenter())+VectorRand()*math.Rand(0,Dist/3.75)+self:HeightCorrection()
	...
	timer.Simple(TimerTable[1],function()
		if(IsValid(self.Owner))then
			if not(TacticalReload)then
				...
				EmptyMag:SetDTBool(3,true) --make it an extra long pistol mag
				EmptyMag:Spawn()
				EmptyMag:Activate()
			else
